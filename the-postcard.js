...
(function(){
  ...
  function draw(){
    ...
    if(PC.tpl==='classic'){
      ...
      c.save();c.translate(W-150,H-150);c.strokeStyle='rgba(168,68,47,.6)';c.lineWidth=4;c.beginPath();c.arc(0,0,72,0,7);c.stroke();c.fillStyle='rgba(168,68,47,.7)';c.textAlign='center';c.font='bold 22px Georgia,serif';c.fillText('✦ '+T('Visité le').replace(' le','')+' ✦',0,-6);c.font='18px Georgia,serif';c.fillText(date,0,22);c.restore();
      wmark(c,W,H);
      ...
      c.fillStyle='#f4d35e';c.font='italic 60px Georgia,serif';c.fillText(T('Bonjour de'),W/2,260);
  ...
  function renderStrip(){
    ...
    s.innerHTML=PC.media.map(function(m,i){return '<div class="pc-thumb'+(i===PC.sel?' on':'')+'" data-i="'+i+'"><img src="'+m.url+'"><button class="pc-rm" data-rm="'+i+'" title="'+T('Retirer')+'">×</button></div>';}).join('')+
      '<label class="pc-thumb pc-add" title="Ajouter une photo">＋<input id="pc-img" type="file" accept="image/*" multiple hidden></label>';
  ...
  function open(a,b){
    ...
    var tpls=[['classic',T('Carte postale')],['polaroid','Polaroid'],['retro',T('Rétro')]];
    $('pc-box').innerHTML='<button class="pc-x" onclick="THEPostcard.close()">✕</button>'+
      ...
      '<div class="pc-row"><input id="pc-nom" placeholder="'+T('Nom du lieu')+'" value="'+(PC.nom||'').replace(/"/g,'&quot;')+'"></div>'+
      '<div class="pc-row"><input id="pc-note" placeholder="'+T('Une note, un souvenir…')+'"></div>'+
      '<div class="pc-tpl">'+tpls.map(function(t){return '<button data-tpl="'+t[0]+'">'+t[1]+'</button>';}).join('')+'</div>'+
      '<div class="pc-act"><button id="pc-share">📤 '+T('Partager')+'</button><button id="pc-dl">⬇️ '+T('Télécharger')+'</button></div>';
  ...
  function share(){
    ...
    else { var a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='carte-postale.png';a.click(); alert(T('Partage direct non supporté — carte téléchargée.')); }
  }
  function init(){
    if($('pc-wrap')) return;
