...
(function(){
  ...
  function fillFicheEdit(el, s){
    ...
    el.innerHTML =
      ...
      '<form class="fe-edit-form" hidden novalidate>'+
        ...
        '<button type="button" class="fe-reset" hidden></button>'+
      '</form>';
    ...
    form.addEventListener('submit', function(e){
      ...
      PatFB.setCorrection(s.id, fields).then(function(){
        ...
        reset.hidden=false; reset.textContent=T('patrimoine.fiche.reinit');
      }).catch(function(){ msg.style.color=''; msg.textContent=T('patrimoine.contrib.erreur'); send.disabled=false; });
    });
    // Réinitialiser : n'apparaît que si une correction existe. Confirmation en 2 clics (aucun dialogue natif).
    var reset=form.querySelector('.fe-reset'), armed=false;
    if(corr(s.id)){ reset.hidden=false; reset.textContent=T('patrimoine.fiche.reinit'); }
    reset.addEventListener('click', function(){
      var msg=form.querySelector('.fc-msg');
      if(!armed){ armed=true; reset.classList.add('armed'); reset.textContent=T('patrimoine.fiche.reinit.confirm'); return; }
      reset.disabled=true; msg.style.color=''; msg.textContent=T('patrimoine.contrib.envoi');
      PatFB.removeCorrection(s.id).then(function(){
        msg.style.color='#5c7a52'; msg.textContent=T('patrimoine.fiche.reinit.ok');
        reset.hidden=true; reset.disabled=false; armed=false; reset.classList.remove('armed');
      }).catch(function(){ msg.style.color=''; msg.textContent=T('patrimoine.contrib.erreur'); reset.disabled=false; armed=false; reset.classList.remove('armed'); reset.textContent=T('patrimoine.fiche.reinit'); });
    });
  }
  ...
  // Lightbox auto-porté : agrandit une photo en plein écran. Ferme au clic hors image, sur ×, ou Échap.
  var _lb=null;
  function openLightbox(url, cred){
    if(!url) return;
    if(!_lb){
      _lb=document.createElement('div'); _lb.className='pat-lightbox'; _lb.style.display='none';
      _lb.innerHTML='<button type="button" class="lb-x" aria-label="×">×</button><img alt=""><figcaption></figcaption>';
      document.body.appendChild(_lb);
      var close=function(){ _lb.classList.remove('on'); setTimeout(function(){ _lb.style.display='none'; },170); };
      _lb.addEventListener('click', function(e){ if(e.target===_lb || e.target.classList.contains('lb-x')) close(); });
      document.addEventListener('keydown', function(e){ if(e.key==='Escape' && _lb.classList.contains('on')) close(); });
    }
    _lb.querySelector('img').src=url;
    var fc=_lb.querySelector('figcaption'); fc.textContent=cred||''; fc.style.display=cred?'':'none';
    _lb.style.display='flex'; void _lb.offsetWidth; _lb.classList.add('on');
  }
  function fillGallery(el, s){
    ...
    if(img) img.addEventListener('click', function(){ openLightbox(img.src, cap?cap.textContent:''); });
