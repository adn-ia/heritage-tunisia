...
### 4.2 ZÉRO TEXTE EN DUR → i18n
Tout libellé passe par le moteur i18n : `data-i18n="clé"` + `ui.<lang>.json` ; le code appelle `uiT('clé')`. Même en une seule langue. Une chaîne visible en dur = ligne rouge.
...
- Livraison : dossier daté `~/Desktop/A-UPLOADER/<App>-<desc>_EST-NNN_date_heure/` + `A-UPLOADER.txt` (cible + ordre) + marqueur `_ref` vérifiable (`curl .../i18n/fr.json | grep EST-NNN`). Seulement les fichiers modifiés. Index `A-UPLOADER/_OU-EST-QUOI.txt` à tenir à jour.
