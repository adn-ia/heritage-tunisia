...
(function () {
  ...
  /* ⇧ EN FAIRE LE POINT DE DÉPART — porté du socle (HERITAGE-SOCLE/roadtrip-plan.js
     l. 109), adapté à la Tunisie sur deux points. ① L'ordre commande les numéros :
     on relève les clés AVANT et on referme APRÈS, comme moveStep — les rangements
     suivent. ② Une VISITE qu'on porte en tête garderait un `baseKey` désignant une
     base passée derrière elle. L'app a déjà le geste : elle redevient une étape
     normale, exactement ce que fait « ⤴ Détacher de la base ». */
  function enFaireLeDepart(i){
    if(!haveRoute()) return;
    var r=LASTRES.route;
    if(i<=0 || i>=r.length) return;
    var s=r[i];
    if(role(s)==="visite") metaPatch(s, {kind:"", baseKey:""});
    var avant=clesActuelles();
    r.splice(i,1); r.unshift(s);
    LASTRES.manualOrder=true;
    /* l'origine du parcours se cale sur elle AVANT recompute() : c'est de
       LASTORIGIN.coord que part le premier tronçon. */
    try{
      if(s.c && s.c.length===2 && typeof LASTORIGIN==='object' && LASTORIGIN){
        LASTORIGIN={ coord:[s.c[0], s.c[1]], label:(s.p&&s.p.nom)||'' };
      }
    }catch(e){}
    recompute();
    if(!renumeroterDepuis(avant)) rerender();
    try{ if(window.THEtoast) THEtoast(T("plan.depart.pose")); }catch(e){}
  }
  function dropStep(i){
    if(!haveRoute()) return;
  ...
  /* La seule porte d'un voyage encore vide. Elle ouvre le MÊME écran de saisie
     que « Ajouter une étape ici », à la position -1 (au début). */
  function porteDeDepart(wrap){
    if(wrap.querySelector(":scope > .rtp-ins")) return;
    css();
    var ins=document.createElement("div"); ins.className="rtp-ins";
    var b=document.createElement("button"); b.type="button";
    b.textContent="＋ "+T("plan.premiere.etape");
    b.onclick=function(){ ouvrirSaisie(-1, null, ins); };
    ins.appendChild(b);
  ...
  function inject(){
    ...
    /* ⚠️ LA PORTE DU VOYAGE VIDE SE POSE AVANT `haveRoute()`, ET C'EST TOUT LE
       POINT — 07/09/2026, deuxième essai. `haveRoute()` rend `route.length`,
       donc **0** quand le voyage n'a encore aucune étape : la fonction sortait
       ici même, avant d'avoir rien posé, et mon premier correctif — placé plus
       bas, après ce garde — ne s'exécutait jamais. Vu à l'écran, pas déduit.
       Un voyage OUVERT mais VIDE existe : LASTRES est là, sa route est vide. */
    ...
    /* ── UN VOYAGE SANS AUCUNE ÉTAPE N'AVAIT PLUS DE PORTE — 07/09/2026 ────────
       Helmy, sur son téléphone : « le voyage libre bloque à la composition des
       étapes, aucune option pour ajouter une étape ». Reproduit à l'écran.

       Les « ＋ Ajouter une étape ici » s'accrochent aux étapes EXISTANTES ; à zéro
       étape il n'y en avait donc aucun — et cette fonction abandonnait ici même,
       avant de rien poser. Le « ➕ Ajouter une étape » du bas de page, lui, avait
       été retiré le 31/08 parce qu'il faisait doublon : il ne l'était plus dans ce
       cas-là. Or un VOYAGE LIBRE commence toujours à zéro étape, et sa carte
       promet en toutes lettres d'en ajouter une. Il ne pouvait plus démarrer.

       ⚠️ La leçon : retirer un doublon ne se vérifie pas sur le cas courant, mais
       sur le cas VIDE — c'est là qu'un geste unique redevient le seul. */
    if(!cards.length){ porteDeDepart(stopsWrap); return; }
    css();
    ...
    cards.forEach(function(card, i){
      ...
      if(!card.querySelector(".rtp-actions")){
        ...
        if(i>0)              item("⇧ "+T("plan.definir.depart"), function(){ enFaireLeDepart(i); });
